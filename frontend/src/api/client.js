...
export const api = {
  ...
  validatePromo: (code, subtotal) => request('POST', '/api/promo/validate', { body: { code, subtotal } }),

  // admin auth
  login: (email, password) => request('POST', '/api/admin/login', { body: { email, password } }),
  ...
  updateProduct: (id, payload) => request('PUT', `/api/products/${id}`, { body: payload, auth: true }),
  toggleProduct: (id, active) => request('PATCH', `/api/products/${id}/status`, { body: { active }, auth: true }),
  ...
  uploadImages: (productId, files) => {
    ...
    return request('POST', `/api/products/${productId}/images`, { body: fd, auth: true, isForm: true });
  },
  ...
  updateOrderStatus: (id, status) => request('PATCH', `/api/admin/orders/${id}/status`, { body: { status }, auth: true }),
  ...
  // admin promo codes
  listPromoCodes: () => request('GET', '/api/admin/promo-codes/admin', { auth: true }),
  createPromoCode: (payload) => request('POST', '/api/admin/promo-codes/admin', { body: payload, auth: true }),
  updatePromoCode: (id, payload) => request('PUT', `/api/admin/promo-codes/admin/${id}`, { body: payload, auth: true }),
  deletePromoCode: (id) => request('DELETE', `/api/admin/promo-codes/admin/${id}`, { auth: true }),
};
